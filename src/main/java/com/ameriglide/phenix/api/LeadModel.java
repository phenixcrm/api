...
public class LeadModel extends ListableModel<Opportunity> {
    ...
    public static JsonMap json(Opportunity o) {
        ...
        if (isNotEmpty(phone)) {
            ...
            if (time==null) {
                ...
                json.put("localTime",
                        ZonedDateTime.ofInstant(Instant.now(), time.getZoneId()).getOffset().getTotalSeconds());
            }
        }
        return json;
    ...
    public Query<Opportunity> all(final Class<Opportunity> type, final HttpServletRequest request) {
        ...
        if (pls!=null && pls.length > 0) {
            query = query.and(
                    Opportunity.withProductLineIdIn(Arrays.stream(pls).map(Integer::valueOf).collect(toList())));
        }
