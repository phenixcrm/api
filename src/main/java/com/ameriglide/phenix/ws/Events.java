...
public class Events extends Endpoint {
    ...
    public static Ticket getTicket(final Session session) {
        return (Ticket) ((HttpSession) session.getUserProperties().get(HttpSession.class.getName())).getAttribute(
                "ticket");
    }
    ...
    public static void broadcast(final String type, final Integer principal, final Json msg) {
        if (principal==null) {
            // tell everyone
            sessions.values().stream().flatMap(Funky::stream).forEach(session -> send(session, type, msg));
        } else {
