...
public class HudHandler implements Runnable, JsonMessageHandler {
    ...
    public void run() {
        ...
        try {
            ...
            for (final var entry : status.entrySet()) {
                ...
                current.put(Integer.toString(entry.getKey()), new JsonMap()
    ...
    private void updateAvailability() {
        forEach(Agent.isActive, agent -> {
            ...
            if (isNotEmpty(agent.getSid())
                    && router.byAgent.computeIfAbsent(agent.getSid(), sid -> router.getWorker(sid).getAvailable())
                    !=hudStatus.available) {
