...
public class Locator {
  ...
  private synchronized static <T> ObjectCache<T> getCache(final Class<T> type) {
    if (cacheableTypes.contains(type)) {
      return (ObjectCache<T>) objectCaches.computeIfAbsent(type,
        cacheType -> ObjectCache.$("Locator", type.getSimpleName(), type));
    }
  ...
  public static <T> Set<T> $$(final Query<T> query) {
    return cacheQuery(query, arg -> collect(query, new AddSortedSet<>()));
  }
  ...
  private static <R, T> R cacheQuery(final Query<T> q, final Function<Query<T>, R> proc,
    ...
    if (queryCache.containsKey(key)) {
      ...
      for (final Class<?> type : q.getTypeDependencies()) {
        queryTypeDependencies.computeIfAbsent(type, t -> new ArrayList<>(8)).add(key);
      }
        ...
        (ResultSet arg) -> {
          ...
          try {
            while (arg.next()) {
              ...
              if (g!=null) {
                var kvMap = map.computeIfAbsent(g, k -> new HashMap<>());
                var k = keyColumn.read(key, arg, jdbc.vendor);
                if (k!=null) {
  ...
  @SuppressWarnings("unused")
  public static <O> Where getVersionCreationWhere(final Class<O> type, final Namer namer, final DateInterval period) {
