package net.inetalliance.potion.info.columns;

import net.inetalliance.potion.info.Column;
import net.inetalliance.sql.DbVendor;
import net.inetalliance.sql.Namer;
import net.inetalliance.sql.SqlType;
import net.inetalliance.sql.Where;
import net.inetalliance.types.json.Json;
import net.inetalliance.types.json.JsonInteger;
import net.inetalliance.types.json.JsonMap;

import java.io.DataInput;
import java.io.DataOutput;
import java.io.IOException;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;

public class IntegerColumn
    extends Column<Integer> {

  public IntegerColumn(final String name, final boolean required, final boolean unique) {
    super(name, required, unique);
  }

	@Override
  public Json toJson(final Integer integer) {
    return new JsonInteger(integer);
  }

  @Override
  public Where getAutoCompleteWhere(final Namer namer, final String table, final String name,
      final String term) {
    try {
      return Where.eq(table, name, Integer.parseInt(term));
    } catch (NumberFormatException e) {
      return null;
    }
  }

  @Override
  public Integer[] newArray(final int size) {
    return new Integer[size];
  }

  @Override
  public Integer read(final String name, final ResultSet r, final DbVendor vendor)
      throws SQLException {
    return r.getInt(name);
  }

  @Override
  public Integer read(final Json json, final Integer previousValue) {
    return json.toInteger();
  }

  @Override
  public Integer read(final DataInput in)
      throws IOException {
    return in.readInt();
  }

  @Override
  public void write(final DataOutput out, final Integer value)
      throws IOException {
    out.writeInt(value);
  }

  @Override
  public SqlType getType(final DbVendor vendor) {
    return SqlType.sqlInteger;
  }

  @Override
  public void bind(final PreparedStatement s, final DbVendor vendor, final Integer index,
      final Integer integer)
      throws SQLException {
    s.setInt(index, integer);
  }

  @Override
  protected void recordDefinitionAdditional(final JsonMap definition) {
    definition.put("type", "int");
    definition.put("align", "right");
    definition.put("defaultValue", "null");
    definition.put("useNull", true);
  }
}
